...
public class GetState extends ActionSupport implements ServletRequestAware{
	...
	public String execute() throws Exception {
		...
		Map<String,Object> result=carrierService.getState(carrier);
		
		String state=(String) result.get("Result");
		
		if(state.equals("Success")) {
			request.setAttribute("State", result.get("State").equals("work")?"工作":"休息");
			request.setAttribute("CurrentUser", account);
			
			String stateOtherChinese=result.get("State").equals("work")?"休息":"工作";
			
			request.setAttribute("StateOtherChinese", stateOtherChinese);
			
			String changeStateAction=result.get("State").equals("work")?"rest.action":"work.action";
